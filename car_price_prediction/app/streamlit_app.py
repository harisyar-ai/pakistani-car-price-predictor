...
def get_fuel_options(model_name): return model_specs.get(model_name, {}).get("fuel", ["Petrol", "Hybrid", "Diesel", "CNG", "Electric"])
def get_transmission_options(model_name): return model_specs.get(model_name, {}).get("transmission", ["Automatic", "Manual"])
...
with col2:
    ...
    city = st.selectbox("City", ["Lahore", "Karachi", "Islamabad", "Rawalpindi", "Faisalabad", "Multan", "Peshawar", "Gujranwala", "Quetta", "Sialkot", "Other"])
    if condition == "New":
        mileage = 0
...
if st.button("Predict Price in Pakistan", type="primary", use_container_width=True):
    df = pd.DataFrame([{
        ...
        'brand_tier': 3 if brand in ["Toyota","Honda"] else (2 if brand in ["Kia","Hyundai","MG","Mercedes","BMW","Audi"] else 1),
        'is_top_model': 1 if model_input.lower() in ["civic","city","corolla","yaris","sportage","tucson","fortuner","revo","grande","prius","aqua","vezel","hr-v"] else 0
    }])
    ...
    try:
        ...
        st.markdown("---")
        st.markdown(f"""
            <h1 style='text-align: center; color: #FF4B4B;'>PKR {pred:,.0f}</h1>
            <h2 style='text-align: center; color: #2E8B57;'>≈ {urdu_price}</h2>
        """, unsafe_allow_html=True)
