...
/* ── HIDE NATIVE SIDEBAR TOGGLE (replaced by our custom button) ── */
...
.stApp {
    ...
    background-image:
        radial-gradient(ellipse at 15% 0%,  rgba(26,111,255,0.05) 0%, transparent 55%),
        radial-gradient(ellipse at 85% 100%, rgba(16,80,204,0.06) 0%, transparent 55%);
...
/* ── TOP HEADER BAR ── */
.pw-header {
    background: linear-gradient(135deg, rgba(247,248,250,0.98) 0%, rgba(234,238,245,0.98) 100%);
...
.pw-logo {
    ...
    text-shadow: 0 0 40px rgba(26,111,255,0.45);
    line-height: 1;
}
...
/* ── INPUT FORM PANEL ── */
.form-panel {
    background: linear-gradient(160deg, rgba(234,238,245,0.9) 0%, rgba(247,248,250,0.95) 100%);
...
.result-box {
    background: linear-gradient(135deg, rgba(26,111,255,0.06) 0%, rgba(247,248,250,0.96) 100%);
    border: 1px solid var(--border2);
    border-radius: 10px;
    padding: 36px 40px;
    text-align: center;
...
.result-price {
    ...
    color: var(--red);
    letter-spacing: 0.04em;
    text-shadow: 0 0 40px rgba(26,111,255,0.3);
    line-height: 1.1;
}
.result-range {
    font-size: 1rem;
...
.conf-mod    { background: rgba(26,111,255,0.10);  color: var(--red2); border: 1px solid var(--border2); }

/* ── CAR LISTING CARD (hover) ── */
.car-card-wrap { text-decoration: none !important; display: block; }
.car-card {
    background: linear-gradient(160deg, rgba(234,238,245,0.92) 0%, rgba(247,248,250,0.96) 100%);
    border: 1px solid var(--border);
    border-radius: 8px;
    overflow: hidden;
...
.car-card::before {
    ...
    background: linear-gradient(160deg, rgba(26,111,255,0.06), transparent);
...
.car-card:hover {
    ...
    box-shadow: 0 20px 50px rgba(0,0,0,0.12), 0 0 30px rgba(26,111,255,0.12);
}
...
.car-card-link {
    ...
    letter-spacing: 0.14em;
    text-transform: uppercase;
    border-bottom: 1px solid rgba(26,111,255,0.25);
    width: fit-content;
    transition: color 0.2s, border-color 0.2s;
}
.car-card:hover .car-card-link { color: var(--red2); border-color: rgba(77,143,255,0.5); }
...
/* ── STREAMLIT WIDGETS — white bg, readable text ── */
.stSelectbox label,
.stNumberInput label,
.stSlider label {
...
/* ── BASEWEB DROPDOWN — white bg, dark text, keep arrow ── */
[data-baseweb="select"] > div {
    background: #FFFFFF !important;
    border-color: #CCCCCC !important;
}
/* Selected value — cover every possible element Streamlit uses */
...
[role="option"][aria-selected="true"] * {
    background: rgba(26,111,255,0.12) !important;
    background-color: rgba(26,111,255,0.12) !important;
    color: #1A6FFF !important;
}
/* Show text cursor inside dropdown input so users know they can type */
[data-baseweb="select"] input,
[data-baseweb="select"] [role="combobox"] {
...
.stButton > button:hover {
    ...
    box-shadow: 0 6px 24px rgba(26,111,255,0.35) !important;
    transform: translateY(-1px) !important;
}

/* ── SIDEBAR TOGGLE BUTTON — always dark & visible ── */
/* Light theme: dark button on white background = clearly visible. */
/* button[kind="header"] is the real Streamlit attribute;          */
/* data-testid selectors are version fallbacks.                    */
...
[data-testid="stSidebar"] {
    background: linear-gradient(180deg, rgba(247,248,250,0.99) 0%, rgba(234,238,245,0.99) 100%) !important;
...
[data-testid="stSidebar"] .stRadio [role="radio"][aria-checked="true"] {
    background: rgba(26,111,255,0.14) !important;
    border-color: var(--border2) !important;
}
[data-testid="stSidebar"] .stRadio [role="radio"] p,
[data-testid="stSidebar"] .stRadio [role="radio"] span,
...
/* ── INSIGHT IMAGE CARD ── */
.insight-card {
    background: linear-gradient(135deg, rgba(234,238,245,0.92), rgba(247,248,250,0.96));
    border: 1px solid var(--border);
    border-radius: 8px;
    padding: 20px;
    margin-bottom: 1.5rem;
...
.about-stat { display: flex; align-items: center; gap: 14px; padding: 11px 0; border-bottom: 1px solid rgba(26,111,255,0.07); }
...
/* ── FOOTER ── */
.pw-footer {
    background: linear-gradient(135deg, rgba(234,238,245,0.99), rgba(220,228,240,0.99));
...
.stInfo { background: rgba(26,111,255,0.04) !important; border: 1px solid var(--border) !important; }
...
.author-hero {
    ...
    align-items: center;
    gap: 36px;
    background: linear-gradient(135deg, rgba(234,238,245,0.92) 0%, rgba(247,248,250,0.96) 100%);
    border: 1px solid var(--border);
    border-radius: 10px;
    padding: 32px 36px;
...
.author-avatar {
    ...
    object-fit: cover;
    flex-shrink: 0;
    box-shadow: 0 0 30px rgba(26,111,255,0.15);
}
...
.social-pill:hover {
    ...
    box-shadow: 0 8px 24px rgba(26,111,255,0.12);
}
...
.author-info-row {
    ...
    gap: 12px;
    padding: 9px 0;
    border-bottom: 1px solid rgba(26,111,255,0.05);
}
...
/* ── CARD GRID: desktop flex row via Streamlit columns (unchanged) ── */
...
/* ── MOBILE SWIPE CAROUSEL for listings ── */
@media (max-width: 768px) {
    .cards-animated [data-testid="stHorizontalBlock"] {
        display: flex !important;
    ...
    .cards-animated [data-testid="stHorizontalBlock"]::-webkit-scrollbar {
        display: none !important;
    }
}
...
@media (max-width: 768px) {
    ...
    /* ── STREAMLIT COLUMNS: force all multi-col layouts to stack (except card carousels) ── */
    [data-testid="stHorizontalBlock"]:not(.cards-animated [data-testid="stHorizontalBlock"]) {
        flex-wrap: wrap !important;
    }
    ...
    /* Swipe card columns override the general column rule above */
    .cards-animated [data-testid="stHorizontalBlock"] > [data-testid="stVerticalBlockBorderWrapper"],
    .cards-animated [data-testid="stHorizontalBlock"] > div[data-testid="column"] {
...
@media (max-width: 480px) {
    ...
    .pw-header { padding: 12px 14px !important; margin: -1rem -0.75rem 2rem -0.75rem !important; }
    ...
    <a href="{url}" target="_blank" class="car-card-wrap">
        <div class="car-card">
            ...
            <div class="car-card-body">
                ...
                <div style="font-family:Rajdhani,sans-serif; color:#1A6FFF; font-weight:700; font-size:1rem; margin-bottom:5px;">{price}</div>
...
# A hidden st.radio is the most reliable Streamlit nav mechanism:
#   • It stays in the DOM at all times (no hide/show tricks needed)
#   • JS in a components.html iframe can .click() its <input> elements
#     directly — Streamlit's own event listener picks that up immediately
#     and triggers a rerun, exactly like the user clicking it
#   • The radio widget is made invisible with CSS so users never see it
...
/* ── Zero out every layer of top spacing ── */
.stApp,
.stAppViewMain,
.main,
.block-container,
...
/* ── Collapse radio to zero — take it OUT of flex flow so gap doesn't apply ── */
...
/* ── Strip ALL margin/padding from the iframe wrapper ── */
div.stCustomComponentV1,
div[data-testid="element-container"]:has(div.stCustomComponentV1),
div[data-testid="stVerticalBlockBorderWrapper"]:has(div.stCustomComponentV1) {
    margin: 0 !important;
    padding: 0 !important;
...
/* Kill any extra wrapper padding around the iframe */
div[data-testid="element-container"]:has(iframe) {
    margin: 0 !important;
    padding: 0 !important;
    line-height: 0 !important;
}
/* ── THE REAL FIX: Streamlit stVerticalBlock uses gap; set to 0 for top elements ── */
/* Target only the root block container's direct vertical block to kill spacing */
.block-container > [data-testid="stVerticalBlock"] {
    gap: 0 !important;
}
/* Restore gap after the 4th child (header, hidden radio wrapper, navbar iframe, then content) */
.block-container > [data-testid="stVerticalBlock"] > div:nth-child(n+4) {
    margin-top: 1rem !important;
}

/* ── KILL the gap above the header and between header & navbar ── */
...
# Hidden radio — drives actual page routing.
# JS will .click() the correct <input type="radio"> option.
_radio_val = st.radio(
    label="nav",
    options=PAGE_KEYS,
    index=PAGE_KEYS.index(st.session_state.get("current_page_key", "predict")),
    key="nav_radio",
    label_visibility="collapsed",
)
# Keep session_state in sync (radio value IS the source of truth now)
if _radio_val != st.session_state.get("current_page_key", "predict"):
    st.session_state.current_page_key = _radio_val
    st.rerun()

current_key = _radio_val
page        = PAGE_FULL[current_key]

# ── Visual navbar rendered in a sandboxed iframe ─────────────────
# components.html iframes CAN run JS and CAN access window.parent.document.
# We render the exact same HTML/CSS as old_claude_app.py, then wire
# each <a> click to .click() the matching hidden radio <input>.

_nav_items_html = ""
for _key, _label in zip(PAGE_KEYS, PAGE_LABELS):
    _cls = "active" if _key == current_key else ""
    _nav_items_html += f'<a class="{_cls}" data-page="{_key}">{_label}</a>\n'
...
html, body {{ background:transparent; overflow:hidden; height:38px; }}

#pw-nav {{
    display:flex; flex-direction:row; flex-wrap:nowrap; align-items:center;
    justify-content:center;
    gap:0; background:#ffffff; border-bottom:1.5px solid #e0e0e0;
    padding:0 1rem; overflow-x:auto; scrollbar-width:none;
    -webkit-overflow-scrolling:touch; width:100%; height:38px;
}}
@media (max-width:768px) {{
    #pw-nav {{ justify-content:flex-start; }}
}}
...
#pw-nav a:hover {{
    color:#1A6FFF; border-bottom-color:#1A6FFF;
    background:rgba(26,111,255,.04);
}}
#pw-nav a.active {{
    color:#1A1A2E; border-bottom-color:#1A6FFF; font-weight:700;
}}
</style>
</head>
<body>
...
<script>
(function() {{
    // ── CLOSE THE GAP: walk up from this iframe in the parent doc
    // and zero out any top-margin/gap on the iframe's wrapper elements
    function closeGap() {{
        var doc = window.parent.document;
        // Find this iframe in the parent
        ...
        // Walk up 8 levels, zero margin-top and padding-top on every wrapper
        var el = thisFrame;
        ...
        // Also set negative margin on the iframe itself to pull it up
        thisFrame.style.setProperty('margin-top', '-7rem', 'important');

        // Also find the stVerticalBlock that contains both the header and nav,
        // and set its gap to 0
        var vblock = doc.querySelector('.block-container [data-testid="stVerticalBlock"]');
        if (vblock) {{
    ...
    closeGap();
    setTimeout(closeGap, 50);
    setTimeout(closeGap, 150);
    setTimeout(closeGap, 400);
    setTimeout(closeGap, 800);
}})();

(function() {{
    var PAGE_KEYS   = {PAGE_KEYS};
    var PAGE_LABELS = {PAGE_LABELS};

    document.querySelectorAll('#pw-nav a').forEach(function(link) {{
        link.addEventListener('click', function(e) {{
            e.preventDefault();
            var pageKey = link.getAttribute('data-page');
            var idx     = PAGE_KEYS.indexOf(pageKey);
            if (idx < 0) return;

            // Find all radio inputs inside the hidden st.radio widget
            var doc    = window.parent.document;
            var inputs = doc.querySelectorAll(
                '[data-testid="stRadio"] input[type="radio"]'
            );

            // inputs are ordered same as PAGE_KEYS
            if (inputs[idx]) {{
                inputs[idx].click();   // Streamlit listens to this natively → reruns
            }}
        }});
    }});
}})();
...
# ══════════════════════════════════════════════════════════════
    ...
    with col1:
        ...
        <div class="about-card">
            ...
            <p style='color:#1A1A2E; font-size:0.95rem; line-height:1.9; margin-bottom:14px;'>
                <strong style='color:#1A6FFF;'>Pakistani Used Car Price Prediction — 2026</strong>
            </p>
            <p style='color:#444; font-size:0.86rem; line-height:1.85; margin-bottom:12px;'>
                This is the first Pakistani used car pricing study built on <strong style='color:#1A1A2E;'>2025–2026 PakWheels data</strong>.
                Prior studies (Asghar et al. 2021, Ahtesham & Zulfiqar 2022) relied on a 2020 Kaggle extract
                where prices are <strong style='color:#1A6FFF;'>65–197% below</strong> current market values —
                making those models structurally obsolete for today's buyers and sellers.
            </p>
            <p style='color:#444; font-size:0.86rem; line-height:1.85; margin:0;'>
                This study introduces <strong style='color:#1A1A2E;'>Generation & trim grade hierarchy</strong> as features for the first time
    ...
    st.markdown("""
    <div style="background:rgba(26,111,255,0.04); border:1px solid rgba(26,111,255,0.12); border-radius:6px;
...
# ══════════════════════════════════════════════════════════════
    ...
    <div class="author-hero">
        <img class="author-avatar"
             ...
             onerror="this.src='https://via.placeholder.com/100/EAF0FF/1A6FFF?text=MH'">
    ...
    st.markdown("""
    <div style="margin-top:1.6rem; background:rgba(26,111,255,0.04); border:1px solid rgba(26,111,255,0.1);
                border-radius:8px; padding:18px 24px; font-size:0.83rem; color:#555; line-height:1.85;">
